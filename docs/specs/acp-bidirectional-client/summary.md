...
• Implemented the ACP/client rewrite in githueber/src/acp/ACPSessionManager.ts. The old OpenCodeHTTPClient path is gone; createACPClient() now builds a single bidirectional client
  that:
  ...
  I added regression coverage in githueber/tests/acp-session-manager.test.ts and updated the high-level docs in githueber/docs/ARCHITECTURE.md and githueber/
  README.md.
  ...
  There were already unrelated local changes in githueber/specs/acp-bidirectional-client/PRD-acp-bidirectional.md and untracked example files under specs/acp-bidirectional-client/
  examples/; I left those as-is.

## Question: ACP Interface

› Explain why the new ACP client implementation in githueber/src/acp/ACPSessionManager.ts looks so different from the one described in
  https://agentclientprotocol.com/protocol/schema and the one implemented in the official example code at githueber/specs/acp-bidirectional-client/examples/client.ts . Why do we
...
• Explored
  └ Search export interface Client|interface Client|class ClientSideConnection|implements Client in acp.d.ts
    ...
    Search createACPClient|ACPClientLike|ACPSessionManager\( in githueber
...
• The implementations are solving different problems at different layers.
  ...
  In the ACP schema and in the official example, acp.Client means “the editor-side callback handler in a full ACP connection.” The example in githueber/specs/acp-bidirectional-cli
  ...
  That example then wraps the handler in new acp.ClientSideConnection(...) on top of an ndJsonStream transport at githueber/specs/acp-bidirectional-client/examples/client.ts:117.
  ...
  Our githueber/src/acp/ACPSessionManager.ts is not that. It defines its own narrow ACPClientLike at githueber/src/acp/ACPSessionManager.ts:18 because the daemon only needs
  ...
  - /global/health and /global/event at githueber/src/acp/ACPSessionManager.ts:109
  - /session and /session/{id}/prompt_async at githueber/src/acp/ACPSessionManager.ts:304
  - /session/{id}/abort at githueber/src/acp/ACPSessionManager.ts:340
  ...
  3. OpenCode exposes HTTP + SSE here, not the SDK’s stream transport.
     ...
     message.part.updated at githueber/src/acp/ACPSessionManager.ts:195. That is protocol-shaped, but not the same transport abstraction as the SDK example.
  4. The daemon intentionally collapses ACP into a smaller internal contract.
     ACPSessionManager only needs session lifecycle signals for GitHub routing, so it maps OpenCode events into sessionPaused / sessionCompleted at githueber/src/acp/
     ACPSessionManager.ts:266 and then updates daemon state at githueber/src/acp/ACPSessionManager.ts:389. It does not need full ACP features like permission RPCs, terminal
  ...
  is also why it has heuristics the schema example does not, like detecting pause via [AWAITING_APPROVAL] at githueber/src/acp/ACPSessionManager.ts:282.
